...
def access_one_stock(ticker: str, ticker_dataframe: DataFrame) -> dict[str, list[Any]]:
    """Test to access the info for one stock.
        Preconditions:
    ...
    final = {str(row.Date.date()): [row.Open, row.Close, row.Volume]
...
def access_all_stocks(tickers: DataFrame) -> Any:
    ...
    for ticker, group in tickers.groupby('Ticker'):
        ...
        for row in group.itertuples(index=False):
            ...
            ticker_info[date] = [row.Open, row.Close, row.Volume]
        all_info[ticker] = ticker_info
    return all_info


def download_tickers(tickers_list: set[str],
    ...
    data = yf.download(tickers_list, threads=False, progress=False, **params)
    ...
    if isinstance(data.columns, pd.MultiIndex):  # Adding more than one ticker
        ...
        data_flattened['Ticker'] = next(iter(tickers_list))
...
if __name__ == '__main__':
    ...
    g.add_vertices_start_end({'GOOGL'}, start='2026-03-01', end='2026-03-06', interval='1d')  # Can do start, end times
